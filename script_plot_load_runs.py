...
def merge_according_to(values, order):
    ...
    for limits in order:
        ...
        if possible_values:
            new_values.append(np.mean(possible_values))
        else:
...
def main():
    models = {}
    runs = {}
    with open("runs_data_compare.csv") as file:
    ...
    for conj in run_groups.values():
        ...
        run_matrix = "IEEE57" if run['run']['r'] == 'false' else "Random net"
        run_text = f"N: {run['run']['N']}, Matrix: {run_matrix}"
        ...
        net = "IEEE57" if run['run']['r'] == 'false' else "Random"
        # if run['run']['r'] is None:
        run_save_name = f"N_{run['run']['N']}_net_{net}"
        plt.savefig('plots_compare_grotas_l2g/MSE_compare_run{}_at{}.png'.format(run_save_name, time_now), dpi=500)
        plt.savefig('plots_compare_grotas_l2g/MSE_compare_run{}.png'.format(run_save_name), dpi=500)
        # plt.show()
